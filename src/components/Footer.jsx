  ...
  return (
    <div className=" bg-slate-900 h-80">
      <div className="border-b border-gray-500 flex justify-around items-center p-5">
        <div className=" w-1/4">
          <img src="https://demo.storeking.xyz/images/required/theme-footer-logo.png" className="w-28 mb-10 " />
          <p className="text-white text-md my-6 ">Subscribe to our newsletter</p>
          <div className=" bg-slate-50 rounded-3xl p-[2px] flex justify-between items-center pl-3">
            <input
              className="text-xs  bg-transparent outline-none h-8 text-black border-none"
              placeholder="Your Email Address"
            />
            <button className="bg-green-500 text-white text-sm px-2 py-1 h-8 rounded-3xl">
              Subscribe
            </button>
          </div>
  ...
  return (
    <div>
      {text.map((item, index) => {
        return (
          <div key={index}>
            <h1 className="text-bold text-xl capitalize text-white ">
              {item.title}
            </h1>
            <div className="flex items-center">
              ...
              <p className="text-sm text-white cursor-pointer hover:text-green-500 py-2  ">
                {item.text}
              </p>
            </div>
          </div>
